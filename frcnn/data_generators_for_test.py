...
def calc_rpn(C, augmented_annotation, width, height, resized_width, resized_height, get_feature_map_size, image):
    # get anchor config
    ...
    max_iou_for_anchor = np.zeros((output_height, output_width, 9)).astype('float32')
    best_bbox_for_anchor = np.zeros((output_height, output_width, 9)).astype('int')
    ...
    #     cv2.rectangle(image, (int(round(gt_bboxes[bbox_idx, 0])),int(round(gt_bboxes[bbox_idx, 2]))),
    #                   (int(round(gt_bboxes[bbox_idx, 1])), int(round(gt_bboxes[bbox_idx, 3]))), (0, 255, 0), 2)
        ...
        for anchor_ratio_idx in range(num_anchor_ratios):
            ...
            for ix in range(output_width):
                ...
                for jy in range(output_height):
                    ...
                    # image_acr = image.copy()
                    # cv2.rectangle(image_acr, (int(round(anchor_x1)), int(round(anchor_y1))), (int(round(anchor_x2)), int(round(anchor_y2))), (0,0,255),2)
                    # cv2.namedWindow('acr', cv2.WINDOW_NORMAL)
                    # cv2.imshow('acr', image_acr)
                    # cv2.waitKey(0)
                    for gt_bbox_idx in range(num_gt_bboxes):
                        ...
                        if max_iou_for_anchor[jy, ix, anchor_ratio_idx + 3 * anchor_scale_idx] < curr_iou:
                            max_iou_for_anchor[jy, ix, anchor_ratio_idx + 3 * anchor_scale_idx] = curr_iou
                            best_bbox_for_anchor[jy, ix, anchor_ratio_idx + 3 * anchor_scale_idx] = gt_bbox_idx
                        ...
                        if curr_iou > best_iou_for_bbox[gt_bbox_idx] or curr_iou > C.rpn_max_overlap:
                            ...
                            tw = np.log(
                                (gt_bboxes[gt_bbox_idx, 1] - gt_bboxes[gt_bbox_idx, 0]) / (anchor_x2 - anchor_x1))
                            th = np.log(
                                (gt_bboxes[gt_bbox_idx, 3] - gt_bboxes[gt_bbox_idx, 2]) / (anchor_y2 - anchor_y1))
    ...
    return np.copy(y_rpn_class), np.copy(y_rpn_regr), max_iou_for_anchor, best_bbox_for_anchor
